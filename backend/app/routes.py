...
def _knowledge_unit_to_dict(unit: KnowledgeUnit) -> dict[str, object]:
    return {
        "id": unit.id,
...
def _knowledge_graph_node_to_dict(node: KnowledgeGraphNode) -> dict[str, object]:
    return {
        "id": node.id,
...
def _knowledge_graph_edge_to_dict(edge: KnowledgeGraphEdge) -> dict[str, object]:
    return {
        "id": edge.id,
...
def get_knowledge_graph(db: Session = Depends(get_db)) -> dict[str, object]:
    ...
    return {
        "nodes": [
            {
                **_knowledge_graph_node_to_dict(node),
                "knowledge_unit_ids": node_to_units.get(node.id, []),
            }
            for node in nodes
        ],
        "edges": [_knowledge_graph_edge_to_dict(edge) for edge in edges],
        "knowledge_units": [
            {
                **_knowledge_unit_to_dict(unit),
